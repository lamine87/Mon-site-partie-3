                ...
                <form action="{{route('backend_artiste_store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            ...
                            <input type="text" class="form-control" id="url_video" name="url video" value="{{old('url_video')}}" placeholder="URL Vidéo (Obligatoire)">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            ...
                            <textarea type="text" class="form-control" name="description" id="description" placeholder="Description (Obligatoire)">{{old('description')}}</textarea>
                        </div>
                    </div>

                <div class="form-row">
                    <div class="form-group col-md-4">
                        <label for="photo_principale">Genre Musical (Catégorie) *</label>
                         <select multiple class="form-control form-control-lg" id="categories" name="categories[]">
                            @foreach($categories as $categorie)
                                <option
{{--                                    {{old('categorie_id') == $categorie->id ? "selected" : ""}}--}}
                                   selected value="{{$categorie->id}}">{{$categorie->nom}}
                                </option>
                            @endforeach
                         </select>
                    </div>
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control form-control-lg" id="countrie_id" name="countrie_id">
                            @foreach($countries as $countrie)
                                    <option
                                        selected value="{{$countrie->id}}"> {{$countrie->nom}}
                                    </option>
                            @endforeach
                        </select>
                     </div>
